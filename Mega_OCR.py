...
# 🔧 پیش‌پردازش برای افزایش دقت
def preprocess_image(img):
    # تبدیل به خاکستری
    gray = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)
    
    # فیلتر دوطرفه برای کاهش نویز
    gray = cv2.bilateralFilter(gray, 9, 15, 15)  # تغییر پارامترها برای تست
    
    # آستانه‌بندی تطبیقی با مقادیر جدید
    th = cv2.adaptiveThreshold(gray, 255, cv2.ADAPTIVE_THRESH_GAUSSIAN_C,
                               cv2.THRESH_BINARY, 11, 5)  # تغییر به Gaussian و مقادیر جدید
...
def extract_text_from_images(images, languages):
    all_text = ""
    lang_str = '+'.join(languages)  # e.g., 'fas+eng'
    for idx, img in enumerate(images, 1):
        processed = preprocess_image(img)
        # تست با PSM 3 (ستون‌های کامل)
...
# 💾 ذخیره در PDF با فونت‌های مناسب
def save_as_pdf(text, output_path):
    c = canvas.Canvas(output_path, pagesize=letter)
    width, height = letter
    text_lines = text.split("\n")
    y = height - 50  # شروع از بالا
    font = 'Vazirmatn' if HAS_VAZ else 'NotoSans' if HAS_NOTO else 'Helvetica'  # اولویت‌بندی فونت‌ها
    c.setFont(font, 12)
...
# === دیالوگ‌های سفارشی برای پیام‌ها ===
...
class LanguageDialog(Toplevel):
    def __init__(self, parent, available_langs, initial_selected=None):
        ...
        # تنظیم دستی scrollregion برای اطمینان از نمایش اسکرول‌بار
        inner.bind("<Configure>", lambda e: canvas.configure(scrollregion=(0, 0, e.width, max(e.height, 800))))

        # قرار دادن کانواس و اسکرول‌بار
        scrollbar.pack(side="right", fill="y")
        canvas.pack(side="left", fill="both", expand=True)

        # اضافه کردن چک‌باکس‌ها برای زبان‌ها
        ...
        # اتصال چرخ ماوس برای اسکرول
        def on_mouse_wheel(event):
            canvas.yview_scroll(int(-1*(event.delta/120)), "units")
        canvas.bind_all("<MouseWheel>", on_mouse_wheel)

        # فریم برای دکمه‌ها (وسط‌چین کردن)
        btn_frame = Frame(self)
        btn_frame.pack(pady=10, fill="x")
        # فریم داخلی برای وسط‌چین کردن دکمه‌ها
...
class OCRApp:
    def __init__(self, root):
        ...
        self.lang_combined = tk.StringVar(value="eng")  # پیش‌فرض
    ...
    def create_widgets(self):
        # رنگ‌ها
        blue_color = "#2534FF"
        red_color = "#FF0000"  # قرمز برای عنوان

        # استایل برای دکمه‌ها و بوردر اکتیو
        style = ttk.Style()
        ...
        # برای align کردن، از padx=20 و fill="x" استفاده می‌کنیم
